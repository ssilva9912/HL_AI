...
def test_registry_selects_text_parser(tmp_path: Path) -> None:
    ...
    parser = build_registry().get_parser(build_metadata(path))
...
def test_registry_selects_pdf_parser(tmp_path: Path) -> None:
...
def test_registry_parses_text_document(tmp_path: Path) -> None:
    ...
    document = build_registry().parse(build_metadata(path))
...
def test_registry_reports_supported_file(tmp_path: Path) -> None:
    ...
    assert build_registry().can_parse(build_metadata(path)) is True
...
def test_registry_reports_unsupported_file(tmp_path: Path) -> None:
    ...
    assert build_registry().can_parse(build_metadata(path)) is False
...
def test_registry_rejects_unsupported_extension(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        ...
        build_registry().parse(build_metadata(path))
